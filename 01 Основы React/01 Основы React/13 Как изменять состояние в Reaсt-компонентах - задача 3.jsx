...
class CalendarEvent extends React.Component {
  ...
  getCurrentUserConfirmationStatus() {
    const curUser = this.state.invited.find(el => el.id === this.state.currentUser)
...
this.getCurrentUserConfirmationStatus !== STATUS_CONFIRMED &&
      this.setState(prevState => ({
        ...prevState,
        invited: this.state.invited.map(el => {
          
          if (el.id === this.state.currentUser) {
            el.status = STATUS_CONFIRMED;
            return el;
          }
          return el;
        })
